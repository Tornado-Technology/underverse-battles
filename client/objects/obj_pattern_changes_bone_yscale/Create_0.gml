...
update = function() {
	...
	if (type_attack == 0) {	
		...
		for (var i = 0; i < 3; ++i) {
			var bone_instance = create_bone(border_instance.x + side + offset * i * 2, border_instance.y + border_instance.down, bone_changes, speed_bone, 2 + sin(pi *i /2), bone_dir, 0);
			bone_instance.set_pulsating_scale(1, 3, speed_count);
		}
	} else if (type_attack == 1) {
		var position_x;
		var position_y = border_instance.y - border_instance.up;
		var direction_bone;
