...
update = function() {
	...
	var j = 2 - _power * 0.1;
	var moving = irandom(1) ? point_in_circle_ : -point_in_circle_;
	while(i <= 360) {
		var x_ = border_instance.x + dcos(angle) * radius;
		var y_ = border_instance.y + -dsin(angle) * radius;
		var bullet_instance = instance_create_depth(x_, y_, fight_depth.bullet_outside_hight, obj_bullet_clubs_timer_jevil, {
...
time_source_update = time_source_create(time_source_game, (80 - (_power * 2)) / 60, time_source_units_seconds, function () {
	update();
}, [], -1);
