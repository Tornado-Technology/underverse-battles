...
update = function() {
	repeat(3) {
		var position_x = border_instance.x - border_instance.left - 6;
		var position_end_x = border_instance.x + border_instance.right + 20;
		...
		thread_instance.scale_speed = 3 + _power * 0.2;
	}
}
...
time_source_update_destroy = time_source_create(time_source_game, period * (repeats + 2) / 60, time_source_units_seconds, function() {
	instance_destroy();	
})