...
head_jevil_sprite = spr_voodoo_head_jevil;
...
add_head = function() {
	if (global.fight_instance == noone || player_num == -1) return;
	var character_index = global.fight_instance.player[1 - player_num].index;
	if (character_index < 0 || character_index >= array_length(head_sprites)) return;
