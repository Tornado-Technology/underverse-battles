...
callback = function () {
	...
	var position_donw_x = border_instance.x + border_instance.right; 

	create_bone(position_x, border_instance.y - border_instance.up, bone,speed_bone, 1, 0, 180);
	create_bone(position_donw_x, border_instance.y + border_instance.down, bone, speed_bone, 1, 180, 0);
...
update_bone = function () {
	...
	var can_scale = fight_random_float(1.4, 2.2);
	var chance = fight_random_integer(1, 100) <= 30;
	if(!use_gravity_attack) {
		chance = fight_random_integer(1, 100) <= (70 +  5 * _power);
		can_scale = fight_random_float(1.6, 3 + _power * 0.1);
	};
	...
	create_bone(border_instance.x + border_instance.right, border_instance.y + border_instance.down, bone, speed_bone, bone_below_scale, 180, 0);
}
