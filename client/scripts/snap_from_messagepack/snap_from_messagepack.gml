...
/// @param {Id.Buffer} buffer - Binary data to be decoded, created by snap_to_binary()
/// @param {Real} offset - Start position for binary decoding in the buffer. Defaults to 0, the start of the buffer
/// @param {Bool} destroyBuffer - Set to <true> to destroy the input buffer. Defaults to <false>
/// @return {Struct} Nested struct/array data decoded from the buffer, using the messagepack standard
function snap_from_messagepack(buffer, offset = 0, destroy_buffer = false) {
    var old_tell = buffer_tell(buffer);
    buffer_seek(buffer, buffer_seek_start, offset);
    
	var result = new __snap_from_messagepack_parser(buffer).root;
    buffer_seek(buffer, buffer_seek_start, old_tell);
    ...
    static read_value = function() {
        ...
        if (_byte <= 0x7f) {
			...
			switch(_byte) {
	            ...
	            case 0xc1: /*192*/ logger.warning("snap_from_binary(): WARNING! Datatype 0xc1 found, but this value should never be used"); break; //Baby shoes for sale, never worn
	            ...
	            default:
	                logger.warning("snap_from_binary(): WARNING! Unsupported datatype " + string(_byte) + " found");
					break;
			}
		}
